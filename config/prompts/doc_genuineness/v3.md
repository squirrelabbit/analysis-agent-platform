---
title: Dataset Document Genuineness (LLOA v3, festival base)
operation: dataset_doc_genuineness
status: active
summary: cleaned doc 단위로 *진성 후기* 여부를 3-tier(genuine_review / non_review / uncertain)로 분류. festival 통합 base — v1(본문) 그대로 두고 행사 고유 예시는 dataset.metadata.doc_genuineness.extra_examples로 주입한다.
notes: |
  - 2026-06-25 — festival 통합 base. doc_genuineness는 v1/v2가 본문·Rules·스키마
    동일하고 Examples 도메인만 달랐다(v1 강릉 / v2 군산). base는 v1 본문·예시를
    그대로 동결하고, 행사 고유 예시/지침은 행사별 추가 슬롯
    ({{extra_instructions}}/{{extra_examples}}, 비면 섹션째 생략)으로 받는다.
  - 군산 dataset은 v2의 군산 예시를 metadata.doc_genuineness.extra_examples로 옮긴다.
  - legacy mixed tier는 제거됨(2026-06-16). 중간 tier는 uncertain.
---
