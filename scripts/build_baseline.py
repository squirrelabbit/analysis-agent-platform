...
LLOA per-call 지연/p95는 worker의 `lloa.call.completed` 로그(2026-06-30 추가)에서 집계한다.
한계(로그 전용): artifact I/O·메모리·stage 내부 분해는 없음(stage wall 시간이 주 신호).
...
def lloa_per_call_from_logs(since_iso: str) -> dict:
    """python-ai-worker `lloa.call.completed` 로그에서 LLOA 호출당 latency 집계.

    silverone 2026-06-30 — LloaClient가 호출당 duration/token을 남기므로 per-call
    count/avg/p50/p95/max(ms) + token 합계를 stage wall과 별개로 본다.
    """
    cmd = COMPOSE.split() + ["logs", "--since", since_iso, "python-ai-worker"]
    try:
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=60).stdout
    except Exception as exc:
        return {"error": str(exc)}
    ...
    for line in out.splitlines():
        ...
        try:
            ...
            continue
        if ev.get("event") != "lloa.call.completed":
            continue
        d = ev.get("duration_ms")
        if isinstance(d, (int, float)):
            durs.append(float(d))
        prompt_toks += int(ev.get("prompt_tokens") or 0)
        comp_toks += int(ev.get("completion_tokens") or 0)
    if not durs:
        return {"samples": 0}
    durs.sort()
    p = lambda q: durs[min(len(durs) - 1, int(q * len(durs)))]
...
def main() -> None:
    ...
    lloa_per_call = lloa_per_call_from_logs(run_started_iso)
    ...
    report = {
        ...
        "lloa_per_call_latency": lloa_per_call,
        "note": "log-only baseline. LLOA per-call latency from lloa.call.completed logs "
                "(stage wall time still the primary signal). artifact I/O / memory / "
                "intra-stage breakdown not captured.",
    }
    ...
    md += ["", f"### LLOA per-call 지연: {json.dumps(lloa_per_call, ensure_ascii=False)}",
           "", f"### build view API 지연: {json.dumps(api_p95, ensure_ascii=False)}",
           "", f"> {report['note']}"]
    md_text = "\n".join(md)
    print("\n" + md_text)
