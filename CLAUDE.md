...
- δ-4 (2026-05-21)로 plan은 planner_v2(LLM)가 plan_v2로 직접 생성하므로 plan skill 카탈로그(`config/skill_bundle.json`)는 삭제됐다. plan_v2 8 skill catalog는 `workers/python-ai/src/python_ai_worker/planner_v2/schema.py`의 `SKILL_CATALOG`로 잠금되어 있다. dataset_build task를 추가·제거하면 `config/task_registry.json`, worker handler, 테스트, `docs/skill/*`를 함께 점검한다.
...
| 컴포넌트 | 역할 |
| --- | --- |
| **Go control plane** (`apps/control-plane`) | HTTP API, dataset build orchestration, analyze_v2 endpoint, Temporal client (dataset build only) |
| **Python AI worker** (`workers/python-ai`) | planner_v2 (LLM), executor_v2 (DuckDB), dataset build |
| **Temporal** | dataset build workflow만 실행. 분석(analyze_v2)은 stateless |
| **Postgres** (pgvector) | project, dataset, dataset_version, artifact 메타데이터 |
| **DuckDB** | executor_v2의 plan_v2 step 실행 엔진 |

### 언어별 책임 경계

- **Go**: API, request validation, dataset build Temporal 호출, analyze_v2 endpoint (LLM 호출은 Python에 위임)
- **Python**: planner_v2 (LLM plan 생성), executor_v2 (DuckDB), dataset build (LLOA / Anthropic)
- **DuckDB**: plan_v2 step 실행 (Python 안에서)

### Analyze 실행 흐름 (analyze_v2, stateless)

```
POST /projects/{pid}/datasets/{did}/analyze_v2                   ← active version
POST /projects/{pid}/datasets/{did}/versions/{vid}/analyze_v2    ← explicit version

  Go control plane
    → active version resolve + artifact_paths 주입
    → Python worker /tasks/analyze_v2 호출
      → planner_v2 (Anthropic) → plan_v2 JSON
      → executor_v2 (DuckDB) → step 순차 실행
    → response body로 plan + 결과 반환 (DB 저장 없음)
```

**plan_v2 / Task Registry**

- plan_v2 — planner_v2가 8 skill (join / filter / aggregate / compare / calculate / sort / present / summarize)과 3 RESERVED input table (docs / clauses / genuineness)로 plan을 생성한다. catalog는 `workers/python-ai/src/python_ai_worker/planner_v2/schema.py:SKILL_CATALOG`로 잠금. δ-4로 `config/skill_bundle.json`은 삭제됐다.
...
- `http/` — 라우팅, Swagger 노출
- `service/` — dataset, dataset_build, analyze_v2 orchestration
...
- `task_router.py` — task name → handler 매핑 (dataset_build + analyze_v2 진입점)
- `planner_v2/` — LLM plan 생성 (`schema.py` / `validator.py` / `prompt.py` / `llm.py`)
- `executor_v2/` — DuckDB plan_v2 step 실행 (`context.py` / `runner.py` / `service.py` / `skills/`)
...
# δ-1 이후 devtools/ 폴더는 삭제됐다. eval/smoke는 scripts/smoke_analyze_v2_*.sh 사용.
```
...
옛 1.x 분석 흐름(SkillPlan / planner v1 / executions / report_drafts / 13 hardcoded plan skill / `skill_bundle.json`)을 모두 제거하고 **planner_v2 + executor_v2 + analyze_v2 endpoint** stateless 흐름으로 대체.
...
| `config/skill_bundle.json` + schema.md | 삭제. plan_v2 카탈로그는 `planner_v2/schema.py:SKILL_CATALOG`로 잠금 |
