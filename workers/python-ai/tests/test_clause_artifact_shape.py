    ...
    def _run(self, sentences: list[str], labels: list[dict]) -> list[dict]:
        ...
        with patch.object(clause_label, "load_config", return_value=fake_config), \
            ...
            clause_label.run_dataset_clause_label({
                ...
                # 5/20 결정으로 default가 genuine_review+uncertain 필터 ON.
...
class ClauseLabelGenuinenessFilterTests(unittest.TestCase):
    ...
    def test_default_filter_includes_uncertain_excludes_non_review(self) -> None:
        # silverone 2026-06-16 — mixed→uncertain 통합. default가 uncertain 포함,
        # non_review만 제외.
        from python_ai_worker.dataset_build.clause_label import _DEFAULT_INCLUDE_GENUINENESS

        self.assertEqual(set(_DEFAULT_INCLUDE_GENUINENESS), {"genuine_review", "uncertain"})
        self.assertIn("uncertain", _DEFAULT_INCLUDE_GENUINENESS)
        self.assertNotIn("non_review", _DEFAULT_INCLUDE_GENUINENESS)

    def test_request_failure_doc_included_by_default_filter(self) -> None:
        ...
        self.assertEqual(include_tiers, {"genuine_review", "uncertain"})
        self.assertEqual(tier_by_doc["doc:fail"], "uncertain")
        # silverone 2026-06-16 — uncertain이 default 포함이라 격리(uncertain) doc도
        # clause_label 대상에 포함된다 (이전엔 skip).
        self.assertIn(tier_by_doc["doc:fail"], include_tiers)
