// 보존용(휴면) 컴포넌트 — 옛 KeywordTab의 키워드 차트(Aspect별 키워드 워드클라우드·표,
// 긍/부정 Top, 키워드 상세표)를 그대로 추출해 둔 것. 현재 KeywordTab은 "절에서 추출된
// 키워드" 표만 보여주며 이 컴포넌트는 렌더하지 않는다. 다시 쓰려면 KeywordTab에서
// <KeywordAspectCharts /> 를 끼우면 된다. (메타/요약 통계는 KeywordTab에 있으므로 제외)
...
export default function KeywordAspectCharts() {
  ...
  // 데이터(요약)가 없으면 렌더하지 않는다 (보존용 휴면 컴포넌트).
