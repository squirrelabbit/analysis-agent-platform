...
- project 단위로 재사용 가능한 분석 시나리오를 등록한다.
...
- `POST /projects/{project_id}/scenarios`, `GET /projects/{project_id}/scenarios`, `GET /projects/{project_id}/scenarios/{scenario_id}`는 현재 sheet 형태의 시나리오를 저장하는 등록 기반이다. 시나리오 실행 연동은 아직 없다.
