...
- `POST /projects/{project_id}/prompts`
  - 프로젝트별 `prepare`, `prepare_batch`, `sentiment`, `sentiment_batch` prompt template 새 버전을 추가한다.
  - 현재 runtime override는 prepare/sentiment build에만 연결된다.
  - body 예시:
    ```json
    {
    ...
    ```
- `GET /projects/{project_id}/prompt_defaults`
- `PUT /projects/{project_id}/prompt_defaults`
  - 프로젝트 기본 `prepare` / `sentiment` prompt version을 설정한다.
  - body 예시:
    ```json
    {
...
- `GET /projects/{project_id}/datasets/{dataset_id}/versions/{version_id}/prepare_preview`
- `GET /projects/{project_id}/datasets/{dataset_id}/versions/{version_id}/prepare_download`
  - 내부 `prepared.parquet`를 analyst-friendly `CSV`로 변환해서 내려준다.
- `GET /projects/{project_id}/datasets/{dataset_id}/versions/{version_id}/sentiment_preview`
- `GET /projects/{project_id}/datasets/{dataset_id}/versions/{version_id}/sentiment_download`
  - 내부 `sentiment.parquet`를 analyst-friendly `CSV`로 변환해서 내려준다.
