...
| `period_timeline` | `{ rows:[{year, role, role_label, target_start, target_end, target_days, festival_start, festival_end}] }` | 분석 개요 — 연도별 대상기간·축제기간 타임라인 (#31) |
...
- `period_timeline` (#31, 2026-07-02): 축제 메타(project.metadata.festival)의 연도별
  대상기간(target_start~target_end)·축제기간(festival_start~festival_end)·역할(role)을 그대로
  싣는다. `role`=base(기준 연도)/compare(비교 연도), `role_label`="기준 연도"/"비교 연도".
  `target_days`=대상기간 총 일수(양끝 포함). 정렬은 기준 연도 먼저, 그다음 연도 내림차순.
